...
# Fork from https://github.com/zhashkevych/spacer
I only changed lib to work with s3 storage for `Ru` region, because minio doesn't work.
...
4) Run `./.bin/spacer export -p <filename prefix> -f <folder name>` to create dump and export it to storage
   OR `./bin/spacer restore` to restore DB from latest dump in your storage bucket
